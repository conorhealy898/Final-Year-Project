...
%{
DC_Gain = K_PL_r * (-zero_PL_r) / (-pole_PL_r);
zero_PL_r = -10;
pole_PL_r = -30;
K_PL_r = DC_Gain * (-pole_PL_r) / (-zero_PL_r);
%}

roll_PL = tf([1 -zero_PL_r], [1, -pole_PL_r])


roll_PL_to_roll_sys_OL = series(roll_PL, steering_and_roll_sys)

figure; 
rlocusplot(roll_PL_to_roll_sys_OL);
hold on;
pole = rlocus(roll_PL_to_roll_sys_OL, K_PL_r);
plot(real(pole),imag(pole),'mx','Markersize',10);
hold off;
title('Root Locus C1*G2');
sgrid;
phi_over_phi_req = feedback(K_PL_r * roll_PL_to_roll_sys_OL, 1)
...
% Motorcycle Digital Controller Emulation
%
[K_PL_s_d, zero_PL_s_d, pole_PL_s_d] = ...
    MPZ_PL(K_PL_s, zero_PL_s, pole_PL_s, T_s, steering_sys, 'Steering Root Locus')

[K_PL_r_d, zero_PL_r_d, pole_PL_r_d] = ...
    MPZ_PL(K_PL_r, zero_PL_r, pole_PL_r, T_s, steering_and_roll_sys, 'Roll Root Locus')
    
K_P_d_d = 25
%}

% Motorcycle Digital Controller Design
%{

[K_PL_s_d, zero_PL_s_d, pole_PL_s_d] = ...
    dtime_PL(steering_sys, omega_n_steering/2, zeta_steering, T_s, 'Steering Phase Lead')

[K_PL_r_d, zero_PL_r_d, pole_PL_r_d] = ...
    dtime_PL(steering_and_roll_sys, omega_n_roll/2, zeta_roll, T_s, 'Roll Phase Lead')
...
% Poster Plots
figure();
plot(time, phi_ref * 180/pi);
hold on; plot(time, phi * 180/pi); hold off;
...
figure();
plot(time, delta_ref * 180/pi);
hold on; plot(time, delta * 180/pi); hold off;
grid on; box off; grid minor;
xlabel('Time [s]');
...
subplot(3,1,2); plot(time, delta * 180/ pi, 'g'); 
grid on; box off;
xlabel('t [s]');
ylabel('\delta [°]');

subplot(3,1,3); plot(time, V_a_s, 'r');
grid on; box off;
xlabel('t [s]');
ylabel('V_a_s [V]');
